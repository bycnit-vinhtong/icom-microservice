...
public class TodoController {
    ...
    Mono<ResponseEntity<TodoResponseDto>> findTodoById(@PathVariable int id) {
//        return todoService.findTodoById(id)
//                .map(todo -> ResponseEntity.ok(todo))
//                .defaultIfEmpty(ResponseEntity.notFound().build());

        return todoService.findTodoDtoById(id)
                .map(todo -> ResponseEntity.ok(todo));
    }
    ...
    Flux<TodoResponseDto> findAllTodos() {
        return todoService.findAllTodoDtos().log();
    }
